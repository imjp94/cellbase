...
class CellFormatter(ABC):
    ...
    def __init__(self, **kwargs):
        unexpected_attrs = [attr for attr in kwargs if attr not in self.attributes()]
        if unexpected_attrs:
            raise AttributeError("Unexpected attribute%s, expecting%s only" % (unexpected_attrs, self.attributes()))
        else:
    ...
    def non_nones(self):
        return [attr for attr in self.attributes() if self.__getattr__(attr)]
    ...
    def __setattr__(self, attr, value):
        if attr not in self.attributes():
            raise AttributeError("Unexpected attribute %s, expecting%s only" % (attr, self.attributes()))
        else:
